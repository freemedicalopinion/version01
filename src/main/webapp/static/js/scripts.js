...
function init_map()
{
	var coordLat = 28.556957;
	var coordLng = 77.376057;
...
$(function()
{
  ...
  /**/
	...
	/* services carousel */
	$('#doc-service-carousel').owlCarousel({
		singleItem: true,
		itemsDesktop: false,
		itemsDesktopSmall: false,
		itemsTablet: false,
...
function pinItShare(url,description,image){

    var width  = 575,
        height = 400,
        left   = ($(window).width()  - width)  / 2,
        top    = ($(window).height() - height) / 2,
        url    = 'http://pinterest.com/pin/create/button/?url='+url+'&description='+description+'&media='+image,
        opts   = 'status=1' +
                 ',width='  + width  +
                 ',height=' + height +
                 ',top='    + top    +
